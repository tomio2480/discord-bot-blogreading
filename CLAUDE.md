...
4. **データ永続化**: `load_data()` / `save_data()`．Sheets の読み書きは 3 回再試行し，失敗時はキャッシュへ倒す
   ...
   - 18:30 の投稿では，Sheets 取得失敗かつキャッシュに両リンクがそろわない場合に 10 秒間隔で最大 3 回再取得する．再取得後も不足していれば未設定表示で投稿する
   - 18:30 の投稿が成功した場合だけ，両リンクと完全スナップショットを消す（19:00 の新規作成に備えた意図的な消去）．Discord 送信自体が失敗した場合は，再送できるようデータを保持する
   - 両リンクがそろった時点の完全スナップショットをローカルキャッシュに保持し，18:30 の Sheets 取得失敗時にのみフォールバックとして使う
   - connpass の 10 分間隔確認は，両リンクが不足している間または未同期の間だけ Sheets へ通信する．connpass が未設定の場合だけ RSS も確認し，両リンクがそろい同期できた後はローカル確認だけで終了する
