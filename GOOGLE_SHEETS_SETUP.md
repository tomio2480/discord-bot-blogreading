...
- **「Google Sheets 読み込みエラー (n/3)」**: 認証情報・スプレッドシートID・共有設定のいずれかが誤っているか、API が一時的に失敗しています。3 回まで自動で再試行します
- **「Google Sheets 保存エラー (n/3)」**: サービスアカウントに編集権限がないか、API が一時的に失敗しています。3 回まで自動で再試行します
- **「Google Sheets への保存に失敗しました」**: 再試行しても保存できませんでした。ローカルファイルのみ更新されています

## フォールバック機能

環境変数が未設定の場合は、ローカルの`data.json`ファイルにデータを保存します。ただし、Northflankでは再起動時にローカルファイルが消えるため、Google Sheetsの設定を推奨します。
...
- 保存に失敗した場合はローカルキャッシュのみ更新し、ログに残します。
