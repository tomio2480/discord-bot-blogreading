...
- 月曜日の指定時刻に自動投稿
  ...
  - 18:15: リマインダー
  - 18:30: ブログリンク集の投稿
    - Google Sheets の取得に失敗した場合は再取得し，それでも不足する場合は未設定表示で投稿
    - 両リンクがそろった時点の完全スナップショットを取得失敗時のフォールバックに使用
  - 18:38: 感想記入の促し
  - 18:42: 画面共有準備の促し
  - 19:00: 次週用 HackMD 作成と投稿
  - リンク付き投稿はリンクプレビュー（埋め込み）を抑止して送信
- connpass RSS 自動取得（10 分間隔）
  - 両リンクが不足している間，またはローカルキャッシュが未同期の間だけ Google Sheets を確認
  - connpass URL が未設定なら，RSS から次の月曜日のイベントを自動検出
  - 両リンクがそろい Sheets と同期できた後は，ローカル確認だけを行い定期通信を停止
